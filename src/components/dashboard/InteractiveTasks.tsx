...
export default function InteractiveTasks({ tasks }: { tasks: Task[] }) {
  ...
  const handleAddTask = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Auto-set deadline to end of today
    const endOfToday = new Date();
    endOfToday.setHours(23, 59, 59, 999);
    formData.append('deadline', endOfToday.toISOString());
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div key={task.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', borderLeft: `4px solid ${isDone ? '#10b981' : isUrgent ? '#ef4444' : '#3b82f6'}`, transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', opacity: isDone ? 0.5 : 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flex: 1 }}>
                <button 
                  ...
                  disabled={isPending}
                  style={{ background: 'none', border: 'none', padding: 0, cursor: isPending ? 'wait' : 'pointer', color: isDone ? '#10b981' : (isUrgent ? '#ef4444' : '#3b82f6'), display: 'flex' }}
                >
      ...
      {!isAdding ? (
        <button onClick={() => setIsAdding(true)} style={{ marginTop: '8px', background: 'rgba(255,255,255,0.05)', border: '1px dashed rgba(255,255,255,0.2)', padding: '14px', borderRadius: '16px', color: '#94a3b8', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.2s' }}>
          <Plus size={18} /> Thêm Task (Hôm nay)
        </button>
      ) : (
        <form onSubmit={handleAddTask} style={{ marginTop: '8px', display: 'flex', gap: '8px', background: 'rgba(0,0,0,0.3)', padding: '12px', borderRadius: '16px', border: '1px solid rgba(59,130,246,0.3)' }}>
          <input name="title" autoFocus placeholder="Tên công việc..." required disabled={isPending} style={{ flex: 1, padding: '10px 14px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: '#fff', fontSize: '0.95rem', outline: 'none' }} />
          <select name="energyLevel" defaultValue="Medium" disabled={isPending} style={{ padding: '0 12px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: '#fff', outline: 'none', cursor: 'pointer' }}>
          ...
          <button type="submit" disabled={isPending} style={{ background: '#3b82f6', color: '#fff', border: 'none', padding: '0 16px', borderRadius: '12px', fontWeight: 600, cursor: isPending ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}>
             Lưu
          </button>
          <button type="button" onClick={() => setIsAdding(false)} disabled={isPending} style={{ background: 'transparent', color: '#94a3b8', border: 'none', padding: '0 12px', borderRadius: '12px', cursor: 'pointer', fontWeight: 600 }}>
            Hủy
          </button>
        </form>
      )}
